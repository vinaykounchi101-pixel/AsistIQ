...
| **Sprint 4** | `feat/backend-messages-storage` | Case notes & messages, visibility rules (`requester_visible` vs `internal_only`), Supabase Storage integration for attachments (10MB/file, 50MB/case limit), magic-byte signature validation, `/api/v1/cases/{case_id}/messages` & `/attachments` endpoints. | ✅ **Implemented & Verified** |
| **Sprint 5** | `feat/backend-ai-engine` | Gemini AIProvider interface, versioned prompt templates (`backend/ai/`), AI case analysis, continuous summary, missing-info detection, AI communication drafts. | ⏳ **Upcoming (Next Sprint)** |
...
## 🎯 Next Steps
1. Run pytest suite and merge `feat/backend-messages-storage` into `dev`.
2. Launch **Sprint 5 (`feat/backend-ai-engine`)**:
   * Implement `GeminiAIProvider` interface with graceful degradation.
